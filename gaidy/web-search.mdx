...
4. **Ответ с цитатами** — модель формирует ответ на основе найденных данных. Для моделей с нативным поиском ссылки на источники возвращаются в `annotations`
...
<Note>
`server_tool_use` и `annotations` присутствуют только при нативном поиске провайдера. Если модель не поддерживает нативный поиск — используется плагин (Exa), и эти поля не возвращаются.
</Note>
