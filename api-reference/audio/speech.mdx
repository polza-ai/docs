...
| `microsoft/mai-voice` | свой дефолт | `voice id` из документации Microsoft. OpenAI-голоса не поддерживаются |
| `google/gemini*tts` | `Kore` | `Kore`, `Aoede`, `Puck`, `Charon`, `Zephyr`, `Achird`, `Algieba`, `Algenib`, `Fenrir`, `Leda`, `Orus`. OpenAI-голос → fallback на `Kore` |
| `hexgrad/kokoro` | `af_bella` | `af_bella`, `af_sarah`, `af_nicole`, `af_sky`, `am_adam`, `am_michael`, `bf_emma`, `bf_isabella`, `bm_george`, `bm_lewis`. OpenAI-голос → fallback на `af_bella` |
